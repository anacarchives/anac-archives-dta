// pdf-generator.js - version finale 04/05/2026
// 6 rectangles, pas de traits internes, tout tient sur une page

import { jsPDF } from "https://cdn.jsdelivr.net/npm/jspdf@2.5.1/+esm";

const PAGE_W = 210;
const M_L    = 15;
const M_R    = PAGE_W - 15;
const CW     = M_R - M_L;
const MID    = M_L + CW / 2;


export function generateAuthorizationPDF(data) {
    const doc = new jsPDF({ unit: 'mm', format: 'a4' });
    doc.setLineWidth(0.4);

    let y = 5;

    // ══════════════════════════════
    // R1 — Vide (entête papier ANAC)
    // ══════════════════════════════
    doc.rect(M_L, y, CW, 40);
    y += 40;

    // ══════════════════════════════
    // R2 — Titre + Numéro + Type
    // ══════════════════════════════
    doc.rect(M_L, y, CW, 38);

    // date en haut à droite — texte seulement, SANS rectangle
    const DX = M_R - 46;
    doc.setFont('helvetica', 'bold').setFontSize(8);
    doc.text('Date/ Dated', DX, y + 5);
    doc.setFont('helvetica', 'normal').setFontSize(11);
    doc.text(data.dateEmission || '', DX + 23, y + 13, { align: 'center' });

    // titre
    doc.setFont('helvetica', 'bold').setFontSize(20);
    doc.text('Autorisation / Authorization', M_L + 3, y + 9);
    doc.setFont('helvetica', 'normal').setFontSize(7.5);
    doc.text('Aurorisation numéro / authorization number:', M_L + 3, y + 15);

    // numéro à gauche
    doc.setFont('helvetica', 'bold').setFontSize(16);
    doc.text(data.numero || '', M_L + 5, y + 26);

    // type
    doc.setFont('helvetica', 'normal').setFontSize(8.5);
    doc.text('Autorisation type / Authorization', M_L + 3, y + 34);
    doc.setFont('helvetica', 'bold').setFontSize(13);
    doc.text(data.type || '', M_L + 70, y + 34.5);

    y += 38;

    // ══════════════════════════════
    // R3 — Info ANAC / Destinataire
    // ══════════════════════════════
    doc.rect(M_L, y, CW, 36);

    doc.setFont('helvetica', 'normal').setFontSize(7.5);
    doc.text("Délivrée par: Agence Nationale de l'Aviation Civile:", M_L + 3, y + 6);
    doc.text("Delivered by: National Civil Aviation Autority:", M_L + 3, y + 12);
    doc.text("Tél/Tel: 00 222 45 24  40 05", M_L + 3, y + 18);
    doc.text("Télécopie/Fax:00 222 45 25 35 78", M_L + 3, y + 24);
    doc.text("Référence/Reference: " + (data.reference || ''), M_L + 3, y + 30);

    doc.text('A/To:', MID + 3, y + 6);
    doc.setFont('helvetica', 'bold').setFontSize(9);
    doc.text(data.destinataire || '', MID + 13, y + 6);

    doc.setFont('helvetica', 'normal').setFontSize(7.5);
    doc.text('Tél/Tel:', MID + 3, y + 13);
    doc.text('Télécopie/Fax:', MID + 3, y + 19);
    doc.text('Date de recéption de la demande/Date of', MID + 3, y + 25);
    doc.text('receipt of request:', MID + 3, y + 30);
    doc.setFont('helvetica', 'bold').setFontSize(8.5);
    doc.text(data.dateReception || data.dateEmission || '', MID + 46, y + 30);

    y += 36;

    // ══════════════════════════════
    // R4 — Visa + Accord + Aéronef
    // ══════════════════════════════
    doc.rect(M_L, y, CW, 45);

    // Visa/SRT — toujours "A" fixe
    doc.setFont('helvetica', 'bold').setFontSize(9.5);
    doc.text('Visa/SRT A', M_L + 3, y + 5);

    // Visa DTA — texte (l'écriture reste visible)
    doc.text('Visa DTA', MID + 3, y + 5);

    // signature du directeur sur Visa DTA (signature fine bleue par dessus le texte comme sur les vrais documents)
    if (data.signatureUrl) {
        try {
            // signature petite et fine à côté du texte "Visa DTA"
            // largeur 35mm, hauteur 10mm, juste à droite du texte
            doc.addImage(data.signatureUrl, 'PNG', MID + 18, y - 2, 35, 10);
        } catch (e) {
            console.warn('Signature non chargée:', e);
        }
    }

    doc.setFont('helvetica', 'normal').setFontSize(8);
    if (data.typeCode === 'SUR') {
        doc.text(
            "Honneur vous notifier notre accord de survol du territoire mauritanien en faveur de l'avion selon les informations ci-après #",
            M_L + 3, y + 12
        );
    } else {
        doc.text(
            "Honneur vous notifier notre accord de survol du territoire mauritanien et l'atterrissage sur le(s)",
            M_L + 3, y + 12
        );
        doc.text(
            "aéroport(s) International de Nouakchott OUM TOUNSY en faveur de(s) avion(s) selon les",
            M_L + 3, y + 17
        );
    }

    // aéronef labels
    doc.setFont('helvetica', 'bold').setFontSize(8);
    doc.text('Aéronef type / aircraft type', M_L + 3, y + 24);
    doc.text('Immatriculation/ Registration', MID + 3, y + 24);

    // valeurs aéronef
    doc.setFont('helvetica', 'bold').setFontSize(13);
    doc.text(data.aeronefType || '', M_L + CW / 4, y + 38, { align: 'center' });
    doc.text(data.immatriculation || '', M_L + 3 * CW / 4, y + 38, { align: 'center' });

    y += 45;

    // ══════════════════════════════
    // R5 — Motif + Dates + Conditions
    // ══════════════════════════════
    const R5H = data.typeCode === 'SUR' ? 60 : 58;
    doc.rect(M_L, y, CW, R5H);

    let iy = y + 6;

    // motif / opérateur / route
    const champs = [
        ['Motif/Motif:', data.motif || ''],
        ['Opérateur/Operator:', data.operateur || ''],
        [data.typeCode === 'SUR' ? 'itinéraire/Itinerary' : 'Route/Route:', data.route || ''],
    ];
    for (const [label, val] of champs) {
        doc.setFont('helvetica', 'bold').setFontSize(8);
        doc.text(label, M_L + 3, iy);
        doc.setFont('helvetica', 'normal').setFontSize(8);
        doc.text(val, M_L + 50, iy);
        iy += 6;
    }

    iy += 1;
    doc.setFont('helvetica', 'bold').setFontSize(8);
    doc.text('Date début de validité/Validity Start Date :', M_L + 3, iy);
    doc.setFont('helvetica', 'normal').setFontSize(8.5);
    doc.text(data.dateDebutValidite || '', M_L + 92, iy);
    iy += 5;

    doc.setFont('helvetica', 'bold').setFontSize(8);
    doc.text('Date fin vlidité/Validity End Date', M_L + 3, iy);
    doc.setFont('helvetica', 'normal').setFontSize(8.5);
    doc.text(data.dateFinValidite || '', M_L + 92, iy);
    if (data.validiteExtension) {
        doc.setFont('helvetica', 'bold').setFontSize(8.5);
        doc.text(data.validiteExtension, M_L + 138, iy);
    }
    iy += 4;

    // conditions
    doc.setFont('helvetica', 'italic').setFontSize(6.5);
    const conds = [
        "Cette autorisation est délivrée sous réserve que/ This permit is issued subject to :",
        "1) Tous les docuemnts de bord de l'aéronef soient en cours de validité pendant l'opération du vol ci-dessus autorisé/ All aircraft 's onboard docuemnts be valid for the operation of authorized above flight;",
        "2) La réglementation aérienne mauritanienne soit scrupuleusement respectée/ Mauritanian Aviation Regulation is scrupulously respected ;",
    ];
    for (const line of conds) {
        doc.text(line, M_L + 3, iy);
        iy += 3;
    }

    if (data.typeCode !== 'SUR') {
        iy += 1;
        doc.text("NB: Le paiement des frais et taxes relatifs à la délivrance de la présente autorisation sont dus dès la signature de celle-ci par l'ANAC.", M_L + 3, iy);
        iy += 3;
        doc.text("NB: The payment of the fees and taxes relating to the issue of this authorization are due as soon as it is signed by the ANAC.", M_L + 3, iy);
    }

    if (data.typeCode === 'SUR') {
        iy += 3;
        doc.setFont('helvetica', 'italic').setFontSize(7.5);
        doc.text('Salutations distinguées Stop', M_L + 3, iy);
        iy += 4;
        doc.text('Nouakchott Mauritanie Stop et Fin', M_L + 3, iy);
    }

    y += R5H;

    // ══════════════════════════════
    // R6 — Distribution + Signature
    // ══════════════════════════════
    const R6H = 297 - y - 5; // on prend tout l'espace restant jusqu'au bas de page
    doc.rect(M_L, y, CW, R6H);

    const dist = [
        '•  MDN/MET', '•  E.M.A.A', '•  Asecna NKC',
        '•  Bureau piste aéroport NKC', '•  CDT Aéroport NKC',
        '•  GEND-RIM, Police, Douane / Aéroport NKC',
        '•  DNAM /AP /C3I', '•  DTA/DAF/DSV/ANAC',
    ];
    doc.setFont('helvetica', 'normal').setFontSize(7.5);
    dist.forEach((item, i) => {
        doc.text(item, M_L + 3, y + 6 + i * 4);
    });

    const SX = MID + 5;
    doc.setFont('helvetica', 'normal').setFontSize(8);
    doc.text('Nom du signataire:', SX, y + 6);
    doc.setFont('helvetica', 'bold').setFontSize(10);
    doc.text(data.signataire || 'AHMED BABA AHMED', SX + 36, y + 6);

    doc.setFont('helvetica', 'normal').setFontSize(8);
    doc.text('Titre:', SX, y + 14);
    doc.setFont('helvetica', 'bold').setFontSize(10);
    doc.text(data.titreSignataire || 'Directeur Général', SX + 12, y + 14);

    doc.setFont('helvetica', 'normal').setFontSize(8);
    doc.text('Signature et cachet:', SX, y + 26);

    return doc;
}


function safeName(s) {
    return (s || '').replace(/[^a-zA-Z0-9-]/g, '_');
}

export function downloadGeneratedPDF(data, existingDoc = null) {
    const doc = existingDoc || generateAuthorizationPDF(data);
    const name = `${safeName(data.numero) || 'autorisation'}_${safeName(data.operateur)}.pdf`;
    doc.save(name);
}

export function getPDFBlob(data, existingDoc = null) {
    const doc = existingDoc || generateAuthorizationPDF(data);
    return doc.output('blob');
}

export function getPDFDataURI(data, existingDoc = null) {
    const doc = existingDoc || generateAuthorizationPDF(data);
    return doc.output('datauristring');
}
