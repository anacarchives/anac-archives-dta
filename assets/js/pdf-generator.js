// pdf-generator.js
// Corrections 05/05/2026 :
// - R1 agrandi (55mm) pour contenir l'entête papier ANAC centrée
// - Liste de distribution supprimée (déjà sur le papier)
// - Tout le texte en gras
// - Signature directeur sur Visa DTA
// - R6 = signature seulement (sans distribution)
...
export function generateAuthorizationPDF(data) {
    ...
    // Agrandi à 55mm pour centrer l'entête physique
    // ══════════════════════════════
    ...
    // titre
    doc.setFontSize(20);
    doc.text('Autorisation / Authorization', M_L + 3, y + 9);
    doc.setFontSize(7.5);
    doc.text('Aurorisation numéro / authorization number:', M_L + 3, y + 15);

    // numéro
    doc.setFontSize(16);
    doc.text(data.numero || '', M_L + 5, y + 25);

    // type
    doc.setFontSize(8.5);
    doc.text('Autorisation type / Authorization', M_L + 3, y + 32);
    doc.setFontSize(13);
    doc.text(data.type || '', M_L + 70, y + 32.5);
    ...
    doc.text("Référence/Reference:", M_L + 3, y + 26);

    doc.setFontSize(7.5);
    doc.text('A/To:', MID + 3, y + 6);
    doc.setFontSize(9);
    doc.text(data.destinataire || '', MID + 13, y + 6);

    doc.setFontSize(7.5);
    doc.text('Tél/Tel:', MID + 3, y + 12);
    doc.text('Télécopie/Fax:', MID + 3, y + 17);
    doc.text('Date de recéption de la demande/Date of', MID + 3, y + 22);
    doc.text('receipt of request:', MID + 3, y + 27);
    doc.setFontSize(8.5);
    doc.text(data.dateReception || data.dateEmission || '', MID + 46, y + 27);
    ...
    // signature du directeur sur Visa DTA
    if (data.signatureUrl) {
        try {
            doc.addImage(data.signatureUrl, 'PNG', MID + 18, y - 2, 38, 10);
        } catch (e) {
            console.warn('Signature:', e);
        }
    }
    ...
    // ══════════════════════════════
    const R5H = data.typeCode === 'SUR' ? 58 : 56;
    ...
    doc.setFontSize(8.5);
    doc.text(data.dateDebutValidite || '', M_L + 92, iy);
    iy += 5;

    doc.setFontSize(8);
    doc.text('Date fin vlidité/Validity End Date', M_L + 3, iy);
    doc.setFontSize(8.5);
    doc.text(data.dateFinValidite || '', M_L + 92, iy);
    if (data.validiteExtension) {
        doc.text(data.validiteExtension, M_L + 138, iy);
    }
    iy += 4;

    // conditions (italic bold)
    ...
    if (data.typeCode === 'SUR') {
        iy += 3;
        doc.setFont('helvetica', 'bolditalic').setFontSize(7.5);
    ...
    doc.setFontSize(8);
    doc.text('Nom du signataire:', SX, y + 8);
    doc.setFontSize(11);
    doc.text(data.signataire || 'AHMED BABA AHMED', SX + 45, y + 8);

    doc.setFontSize(8);
    doc.text('Titre:', SX, y + 18);
    doc.setFontSize(10);
    doc.text(data.titreSignataire || 'Directeur Général', SX + 14, y + 18);

    doc.setFontSize(8);
    doc.text('Signature et cachet:', SX, y + 30);
